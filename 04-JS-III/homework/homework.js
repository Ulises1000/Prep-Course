...
function agregarNumeros(numeros) {
  ...
  numeros.forEach(elemento => (sum += elemento));
...
function promedioResultadosTest(resultadosTest) {
  ...
  resultadosTest.forEach(elemento => (resultados += elemento));
...
function numeroMasGrande(numeros) {
  ...
  numeros.forEach((elemento,index) => {
    if(index === 0) mayor = elemento;
    else if(mayor < elemento) mayor = elemento;
  });
...
function cuentoElementos(arreglo){
  ...
  arreglo.forEach(elemento => (elemento > 18 && ++cantidad));
...
function diaDeLaSemana(numeroDeDia) {
  ...
  //Escribe tu código aquí   
  return (() => (
    numeroDeDia === 7 || numeroDeDia === 1 ? "Es fin de semana" : "Es dia Laboral"
  ))();
} 
...
function todosIguales(arreglo) {
  ...
  for(let i = 0; i < arreglo.length; i++){
    if(i === 0) igual = arreglo[i];
    else if(igual != arreglo[i]) return false;
  }
...
function mesesDelAño(array) {
  ...
  return array.filter(elemento => (elemento === "Enero" || elemento === "Marzo" || elemento === "Noviembre"));
}
...
function breakStatement(numero) {
  ...
  for(let i = 1; i <= 10; i++){
    ...
    if((numero + 2) === 10){
      quebro = true;
      break;
    }else array.push(numero += 2);
  
  }
  
  return quebro ? "Se interrumpió la ejecución" : array;
}
